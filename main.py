...
referral_system = {}
ratings = {}
rated_orders = {}  # Отслеживание оцененных заказов по пользователям
raffle_numbers = {}
orders = {
    'active': [],
    'accepted': [],  # Новое состояние для принятых, но неоплаченных заказов
    'completed': [],
    'cancelled': []
}
...
💡 For every 10 orders of your referrals you get a gift!"""
        ...
        user_id = str(update.effective_user.id)
        last_game = context.user_data.get('last_game_time')
        now = datetime.now()
        ...
        context.user_data['last_game_time'] = datetime.now()

        # Сохраняем информацию об открытом сундуке
        if 'opened_chests' not in context.user_data:
            context.user_data['opened_chests'] = []
        if 'gifts' not in context.user_data:
        ...
        if not any(keyword in prize for keyword in empty_chest_keywords):
            context.user_data['gifts'].append({
                'description': prize,
                'source': 'chest',
        ...
        context.user_data['opened_chests'].append({
            'date': datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            'prize': prize,
...
Thank you for your order! 🙏"""
                ...
                await query.edit_message_text(confirmation_text_map[lang])
            except Exception as e:
...
For confirmation, send payment screenshot to {ADMIN_CONTACT}"""
                ...
                await query.edit_message_text(simple_text_map[lang])
        ...
        user_id = str(update.effective_user.id)
        # Здесь должна быть логика получения подарков пользователя.
        # Например, из базы данных или из context.user_data
